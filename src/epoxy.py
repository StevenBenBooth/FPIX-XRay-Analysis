...
from interpolate_near_tube import process_highlights
...
def find_epoxy(img, img_tube, tube_radius, slice_num, num_wedges):
    ...
    tube_mask = np.zeros(img_epoxy_raw.shape[:2], np.uint8)  # This mask is just the tube
    ...
    img_epoxy = remove_fiber(top_bound, bottom_bound, cf_thickness, img_epoxy_ntube, fiber_close_ker, open_ker)
    ...
    img_highlights = cv2.bitwise_and(cv2.inRange(img_blur, epox_hlght_cutoff, 255), not_tube_mask)

    # The parameters on this one are pretty tricky. I think (9, 0.7, 0, interpolate_close_ker) is a good starting place
    img_interpolated = process_highlights(img_epoxy, img_highlights, outer_tube, slice_num, num_wedges, 9, 0.6,
                                          0, interpolate_close_ker)
